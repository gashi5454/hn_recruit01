            ...
            $offers = $offersQuery->where(function($query)use($search_words){
                ...
                foreach($search_words as $search_word) {
                    if($firstCondition == true) {
                        ...
                        $query->orWhere('name', 'like', self::escapeLike($search_word));
                    }
                    $query->orWhere('title', 'like', self::escapeLike($search_word));
                    $query->orWhere('genre', 'like', self::escapeLike($search_word));
                    $query->orWhere('contents', 'like', self::escapeLike($search_word));
                    $query->orWhere('place', 'like', self::escapeLike($search_word));
                    $query->orWhere('address', 'like', self::escapeLike($search_word));
                    $query->orWhere('appear_date', 'like', self::escapeLike($search_word));
                    $query->orWhere('postal_code', 'like', self::escapeLike($search_word));
                    $query->orWhere('requirement', 'like', self::escapeLike($search_word));
                }
            });
        }
