...
class OffersController extends BaseController
{
    ...
    public function search(Request $request)
    {
        ...
        {
            foreach($strArry as $value) {
                $offers = $offersQuery->Where('name','like', '%' . self::escapeLike($value) . '%')->orWhere('title','like', '%' . self::escapeLike($value) . '%')->orWhere('genre','like', '%' . self::escapeLike($value) . '%')
            ...
            ->where(function($query)use($search_words){
                ...
                foreach($search_words as $search_word) {
                    $query->where('number_of_bands', 'like', $search_word);
                }
            })->orwhere(function($query)use($search_words){
                foreach($search_words as $search_word) {
                ...
                foreach($search_words as $search_word) {
                    $query->where('tel', 'like', $search_word);
                }
            })->orwhere(function($query)use($search_words){
                foreach($search_words as $search_word) {
                    $query->where('other_contact', 'like', $search_word);
                }
            })->orwhere(function($query)use($search_words){
                foreach($search_words as $search_word) {
                ...
                foreach($search_words as $search_word) {
                    $query->where('capacity', 'like', $search_word);
                }
            })->orwhere(function($query)use($search_words){
                foreach($search_words as $search_word) {
                    $query->where('quota', 'like', $search_word);
                }
            })->orwhere(function($query)use($search_words){
                foreach($search_words as $search_word) {
        ...
        elseif(empty($keyword) && empty($appear_date_start) && !empty($appear_date_end) && empty($address) && empty($genre))
        {
            $query = $query->where('appear_date','<=', $appear_date_end);
            $counts = $query->count();
        ...
        elseif(empty($keyword) && empty($appear_date_start) && empty($appear_date_end) && empty($address) && !empty($genre))
        {
            $offers = $query->where('genre','like', '%' . $genre . '%');
            $counts = $query->count();
        ...
        elseif(!empty($keyword) && !empty($appear_date_start) && empty($appear_date_end) && empty($address) && empty($genre))
        {
            foreach($wordArraySearched as $value) {
                $query = $query->Where('name','like', '%' . self::escapeLike($value) . '%')->orWhere('title','like', '%' . self::escapeLike($value) . '%')->orWhere('genre','like', '%' . self::escapeLike($value) . '%')
                    ->orWhere('number_of_bands','like', '%' . self::escapeLike($value) . '%')->orWhere('contents','like', '%' . self::escapeLike($value) . '%')->orWhere('place','like', '%' . self::escapeLike($value) . '%')
                    ->orWhere('tel','like', '%' . self::escapeLike($value) . '%')->orWhere('other_contact','like', '%' . self::escapeLike($value) . '%')->orWhere('address','like', '%' . self::escapeLike($value) . '%')
                    ->orWhere('postal_code','like', '%' . self::escapeLike($value) . '%')->orWhere('capacity','like', '%' . self::escapeLike($value) . '%')->orWhere('quota','like', '%' . self::escapeLike($value) . '%')
                    ->orWhere('requirement','like', '%' . self::escapeLike($value) . '%');
