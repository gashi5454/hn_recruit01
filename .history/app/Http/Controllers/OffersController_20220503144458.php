...
class OffersController extends BaseController
{
    ...
    //曜日取得
    public function getDateAttribute($appear_date)
    {
        $carbon = new Carbon($appear_date);
        $date = $carbon->isoFormat( 'YYYY年M月D日(ddd)' );

        // 2019年04月06日 19:48:51 (Sat)
        dd($date);

        // "2時間後"
        // dd($carbon->now()->addHours(3)->diffForHumans());
        return $date;
    }
    ...
    public function search(Request $request)
    {
        ...
        elseif(!empty($keyword) && empty($appear_date_start) && empty($appear_date_end) && empty($address) && empty($genre))
        {
            ...
            $date = $offers->appear_date . $this->getDateAttribute($offers->appear_date);
            $offers->appear_date = $date;
            return view('search')->with([
                'offers' => $offers,
                'message' => $message,
            ]);
