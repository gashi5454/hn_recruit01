...
class OffersController extends BaseController
{
    ...
    public function search(Request $request)
    {
        ...
        {
            $offers = $offersQuery->sortable();
        }

        //フリーワードのみ
        elseif(!empty($keyword) && empty($appear_date_start) && empty($appear_date_end) && empty($address) && empty($genre))
        {
            $offers = $offersQuery->sortable()->where(function($query)use($search_words){
                $firstCondition = true;
        ...
        elseif(empty($keyword) && !empty($appear_date_start) && empty($appear_date_end) && empty($address) && empty($genre))
        {
            $offers = $offersQuery->sortable()->where('appear_date','>=', $appear_date_start);
        }

        //以前の日付のみ
        elseif(empty($keyword) && empty($appear_date_start) && !empty($appear_date_end) && empty($address) && empty($genre))
        {
            $offers = $offersQuery->sortable()->where('appear_date','<=', $appear_date_end);
        }

        //地域のみ
        elseif(empty($keyword) && empty($appear_date_start) && empty($appear_date_end) && !empty($address) && empty($genre))
        {
            $offers = $offersQuery->sortable()->where('address','like', '%' .$address. '%');
        }

        //ジャンルのみ
        elseif(empty($keyword) && empty($appear_date_start) && empty($appear_date_end) && empty($address) && !empty($genre))
        {
            $offers = $offersQuery->sortable()->where('genre','like', '%' . $genre . '%');
        }
        ...
        elseif(empty($keyword) && !empty($appear_date_start) && !empty($appear_date_end) && empty($address) && empty($genre))
        {
            $offers = $offersQuery->sortable()->whereBetween('appear_date', [$appear_date_start, $appear_date_end]);
        }
        ...
        elseif(empty($keyword) && empty($appear_date_start) && !empty($appear_date_end) && !empty($address) && empty($genre))
        {
            $offers = $offersQuery->sortable()->where('address','like', '%' . $address . '%')->where('appear_date','<=', $appear_date_end);
        }
