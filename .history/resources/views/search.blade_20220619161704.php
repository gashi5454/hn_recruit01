...
<div class="relative flex items-top justify-center h-auto bg-gray-100 dark:bg-white sm:items-center py-4 sm:pt-0">
    <div class="py-6 mx-auto xs:px-4 min:px-4 sm:px-6 md:px-6 lg:px-6">
        <div class="fixed max-w-800 w-full bg-white p-1 dark:bg-gray-100 overflow-hidden shadow min:rounded-lg">
            <div class="m-2 flex items-center xs:flex-wrap min:flex-wrap sm:flex-wrap md:flex-nowrap lg:flex-nowrap">
                <x-jet-label for="keyword" value="{{ __('フリーワード') }}" />
                <x-jet-input id="keyword" class="mt-1 text-sm w-full max-w-260" placeholder="キーワードを入力" type="text" name="keyword" />
            </div>
            <div class="m-2 flex items-center xs:flex-wrap min:flex-wrap sm:flex-wrap md:flex-nowrap lg:flex-nowrap">
                <x-jet-label for="appear_date" value="{{ __('出演日') }}" />
                <x-jet-input id="appear_date_start" class="block mt-1 mr-2 text-sm" type="date" name="appear_date_start" />
                <div class="mt-2">～</div>
                <x-jet-input id="appear_date_end" class="block mt-1 ml-2 text-sm" type="date" name="appear_date_end" />
            </div>
        </div>
