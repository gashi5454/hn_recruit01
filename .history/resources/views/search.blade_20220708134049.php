    ...
    {{method_field('post')}}
        <div x-data="{isShow:false}" class="mx-auto w-full max-w-760 border-01 shadow p-1 overflow-hidden">
            <div class="xs:px-2 min:px-2 sm:px-6 md:px-6 lg:px-6 py-2 mt-2 flex items-center">
                @isset($keyword)
                    <x-jet-input id="keyword" value="{{ old('keyword', $keyword) }}" class="text-sm w-full min-w-200 max-w-450" placeholder="キーワードを入力" type="text" name="keyword" />
                @else
                    <x-jet-input id="keyword" value="{{ old('keyword') }}" class="text-sm w-full min-w-200 max-w-450" placeholder="キーワードを入力" type="text" name="keyword" />
                @endisset
                <button style="height:38px; min-width:60.5px; max-width:61px;" @click="isShow= !isShow" type="button" class="text-sm ml-4 w-full inline-flex items-center px-4 py-1 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 active:text-gray-800 active:bg-gray-50 disabled:opacity-25 transition">
                        {{ __('絞込') }}
                </button>
            </div>
