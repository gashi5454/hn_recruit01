...
<div class="flex h-auto bg-white py-4">
    ...
    {{ csrf_field()}}
    {{method_field('post')}}
        <div class="mx-auto w-full border-01 shadow p-1 overflow-hidden">
            <div class="flex items-center flex-wrap">
                ...
                <div class="px-2 py-1 w-full">
                    ...
                    <div class="flex">
                        @isset($appear_date_start)
                            <x-jet-input id="appear_date_start" value="{{ old('appear_date_start', $appear_date_start) }}" class="block mt-1 mr-2 w-full xs:max-w-120 text-xs min:max-w-120 text-xs sm: max-w-120 text-xs md:max-w-120 text-xs lg:max-w-120 text-xs" type="date" name="appear_date_start" />
                        @else
                            <x-jet-input id="appear_date_start" value="{{ old('appear_date_start') }}" class="block mt-1 mr-2 w-full xs:max-w-120 text-xs min:max-w-120 text-xs sm: max-w-120 text-xs md:max-w-120 text-xs lg:max-w-120 text-xs" type="date" name="appear_date_start" />
                        @endisset
                        <div class="mt-3">～</div>
                        @isset($appear_date_end)
                            <x-jet-input id="appear_date_end" value="{{ old('appear_date_end', $appear_date_end) }}" class="block mt-1 ml-2 w-full xs:max-w-120 text-xs min:max-w-120 text-xs sm: max-w-120 text-xs md:max-w-120 text-xs lg:max-w-120 text-xs" type="date" name="appear_date_end" />
                        @else
                            <x-jet-input id="appear_date_end" value="{{ old('appear_date_end') }}" class="block mt-1 ml-2 w-full xs:max-w-120 text-xs min:max-w-120 text-xs sm: max-w-120 text-xs md:max-w-120 text-xs lg:max-w-120 text-xs" type="date" name="appear_date_end" />
                        @endisset
                    </div>
                </div>
                <div class="px-2 py-1 w-full">
                ...
                <div class="px-2 py-1 w-full">
                    ...
                    <select id="genre" class="xs:text-sm min:text-sm sm:text-base block mt-1 w-auto border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" name="genre">
            ...
            <div class="flex items-center xs:flex-wrap min:flex-wrap sm:flex-wrap md:flex-wrap lg:flex-nowrap">
                ...
                <div class="px-2 py-2">
                    <select id="disp_list" name="disp_list" onchange="changeListView();" class="xs:text-sm min:text-sm sm:text-base border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
            ...
            <div class="flex items-center justify-center pb-4">
                <x-jet-button id="searchBtn" class="text-base ml-24">
                    {{ __('検索') }}
                </x-jet-button>
                <input id="clearBtn" class="text-xs text-gray-600 ml-2" type="button" value="検索条件をクリア" onclick="clearSearchInfo()" />
            </div>
        </div>
    </form>
        ...
        <div class="w-full p-2 my-8 bg-white border-01 shadow overflow-hidden">
