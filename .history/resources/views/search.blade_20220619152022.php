...
<div class="relative flex items-top justify-center h-auto bg-gray-100 dark:bg-white sm:items-center py-4 sm:pt-0">
    <div class="py-6 mx-auto xs:px-4 min:px-4 sm:px-6 md:px-6 lg:px-6">
        <div class="bg-white dark:bg-gray-100 overflow-hidden shadow min:rounded-lg">
            <div class="p-1 text-sm">
                <div class="m-2 flex flex-wrap items-center" style="flex-direction: column;">
                    <x-jet-label for="keyword" value="{{ __('フリーワード') }}" />
                    <x-jet-input id="keyword" class="mt-1" placeholder="キーワードを入力" type="text" name="keyword" />
                </div>
                <div class="m-2 flex items-center" style="flex-direction: column;">
                    <x-jet-label for="appear_date" value="{{ __('出演日') }}" />
                    <x-jet-input id="appear_date_start" class="block mt-1 mr-2" type="date" name="appear_date_start" />
                    <div class="mt-2">～</div>
                    <x-jet-input id="appear_date_end" class="block mt-1 ml-2" type="date" name="appear_date_end" />
                </div>
            </div>
        </div>
