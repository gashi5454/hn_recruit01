...
<div class="flex h-auto bg-white py-4">
    ...
    {{ csrf_field()}}
    {{method_field('post')}}
        <div x-data="{isShow:false}" class="flex items-center mx-auto w-full max-w-760 border-01 shadow p-1 overflow-hidden">
            <div class="xs:px-2 min:px-2 sm:px-6 md:px-6 lg:px-6 py-2">
                @isset($keyword)
                    <x-jet-input id="keyword" value="{{ old('keyword', $keyword) }}" class="mt-1 text-sm w-full min-w-200 max-w-450" placeholder="キーワードを入力" type="text" name="keyword" />
                @else
                    <x-jet-input id="keyword" value="{{ old('keyword') }}" class="mt-1 text-sm w-full min-w-200 max-w-450" placeholder="キーワードを入力" type="text" name="keyword" />
                @endisset
                <x-jet-secondary-button class="text-sm" type="button" @click="isShow= !isShow">
                        {{ __('詳細検索') }}
