<x-jet-form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        <div class="bands-info mx-auto xs:max-w-280 min:max-w-280 sm:max-w-450 md:max-w-450 lg:max-w-520">
            <x-jet-label for="other_contact" value="{{ __('あささ') }}" />
            <div class="mt-4 text-gray-900 dark:text-gray-900 xs:text-sm min:text-sm sm:text-base md:text-base lg:text-base">人数</div>
            <select id="asasa_num" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm xs:text-sm min:text-sm sm:text-base md:text-base lg:text-base" style="width: 150px; height: 40px;" wire:model.defer="state.asasa_num">
            @foreach(config('bands_num') as $index => $name)
                @if (!is_null(old('asasa_num')))
                    @if ($index == old('asasa_num'))
                        <option value="{{ $index }}" selected>{{ $name }}</option>
                    @else
                        <option value="{{ $index }}">{{ $name }}</option>
                    @endif
            ...
            <div class="mt-4 text-gray-900 dark:text-gray-900 xs:text-sm min:text-sm sm:text-base md:text-base lg:text-base">コーラスマイク本数</div>
            <select id="asasa_mic" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm xs:text-sm min:text-sm sm:text-base md:text-base lg:text-base" style="width: 150px; height: 40px;" wire:model.defer="state.asasa_mic">
            @foreach(config('bands_mic') as $index => $name)
                @if (!is_null(old('asasa_mic')))
                    @if ($index == old('asasa_mic'))
                        <option value="{{ $index }}" selected>{{ $name }}</option>
                    @else
                        <option value="{{ $index }}">{{ $name }}</option>
                    @endif
