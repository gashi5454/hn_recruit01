                ...
                <div x-data="{isShow:false}" class="pt-6">
                    ...
                    <div x-show="isShow" x-cloak>
                        ...
                        <div class="bands-info mx-auto xs:max-w-280 min:max-w-280 sm:max-w-450 md:max-w-450 lg:max-w-520">
                            ...
                            <select id="other_part_num" class="border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm xs:text-sm min:text-sm sm:text-base md:text-base lg:text-base" style="width: 150px; height: 40px;" name="other_part_num">
                            @foreach(config('bands_num') as $index => $name)
                                @if (!is_null(old('other_part_num')))
                                    @if ($index == old('other_part_num'))
                                        <option value="{{ $index }}" selected>{{ $name }}</option>
                                    @else
                                        <option value="{{ $index }}">{{ $name }}</option>
                                    @endif
