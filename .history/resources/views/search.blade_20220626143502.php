            ...
            <div class="flex items-center xs:flex-wrap min:flex-wrap sm:flex-wrap md:flex-wrap lg:flex-nowrap">
                <div class="px-2 py-1 w-full">
                    ...
                    @if (is_null(old('keyword')))
                        @isset($keyword)
                            <x-jet-input id="keyword" value="{{ $keyword }}" class="mt-1 text-sm w-full min-w-200" placeholder="キーワードを入力" type="text" name="keyword" />
                        @else
                        <x-jet-input id="keyword" class="mt-1 text-sm w-full min-w-200" placeholder="キーワードを入力" type="text" name="keyword" />
                        @else
                </div>
                <div class="px-2 py-1 w-full">
                    <x-jet-label for="appear_date" value="{{ __('出演日') }}" />
                    <div class="flex">
                        <x-jet-input id="appear_date_start" class="block mt-1 mr-2 xs:text-xs min:text-xs sm:text-sm md:text-sm lg:text-sm w-full max-w-130" type="date" name="appear_date_start" />
                        <div class="mt-3">～</div>
                        <x-jet-input id="appear_date_end" class="block mt-1 ml-2 xs:text-xs min:text-xs sm:text-sm md:text-sm lg:text-sm w-full max-w-130" type="date" name="appear_date_end" />
                    </div>
                </div>
                <div class="px-2 py-1 w-full">
